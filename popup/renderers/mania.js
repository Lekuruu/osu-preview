...
const getMainBPM = (timingPoints, hitObjects) => {
  ...
  const mainBpm = 60000 / [...bpms.entries()]
    .reduce(([mainMsPerBeat, maxCount], [msPerBeat, count]) => (
      count > maxCount ? [msPerBeat, count] : [mainMsPerBeat, maxCount]), [0, 0])[0];
  return mainBpm;
};
...
const drawHoldNote = (ctx, object, keyCount, time, mainBPM, timingPoints) => {
  ...
  const startOffset = computeSVTimeOffset(
    time,
    object.time,
    mainBPM,
    timingPoints,
  );
  const holdOffset = computeSVTimeOffset(
    Math.max(time, object.time),
    object.endTime,
    mainBPM,
    timingPoints,
  );
